...
export const authApi = {
  ...
  pendingUsers: () => request<{ users: PendingUser[] }>("/auth/pending-users"),
...
export const warehouseApi = {
  ...
  update: (id: string, body: Omit<Warehouse, "_id" | "createdAt" | "updatedAt">) =>
...
export const locationApi = {
  ...
  update: (id: string, body: { name: string; shortCode: string; warehouse: string }) =>
...
export const productApi = {
  ...
  update: (id: string, body: { name: string; skuCode: string; unitCost: number }) =>
    request<{ product: Product }>(`/products/${id}`, { method: "PUT", body: JSON.stringify(body) }),
  updateStock: (id: string, onHand: number) =>
    request<{ product: Product }>(`/products/${id}/stock`, { method: "PATCH", body: JSON.stringify({ onHand }) }),
...
export const operationApi = {
  ...
  create: (body: CreateOperationBody) =>
    request<{ operation: Operation; lines: OperationLine[] }>("/operations", { method: "POST", body: JSON.stringify(body) }),
  update: (id: string, body: UpdateOperationBody) =>
    request<{ operation: Operation; lines: OperationLine[] }>(`/operations/${id}`, { method: "PUT", body: JSON.stringify(body) }),
  todo: (id: string) =>
...
export interface OperationLine {
  _id: string;
  operation: string;
  product: { _id: string; name: string; skuCode: string; unitCost: number; onHand: number };
  quantity: number;
  isShort: boolean;
}
